...
function GetPdfMake(invoice, javascript, callback) {
    ...
    if (!dd.defaultStyle) dd.defaultStyle = {
        font: NINJA.bodyFont
    };
    ...
    if (window.accountBackground) {
        ...
        if (!origBackground) {
            origBackground = [{
                "image": window.accountBackground,
                "alignment": "center"
            }];
        }
...
NINJA.statementDetails = function (invoice) {
    ...
    clone.columns[1].table.body = [
        [{
                ...
                style: ['subtotalsLabel', 'subtotalsBalanceDueLabel']
            },
            {
                text: formatMoneyInvoice(invoice.balance_amount, invoice),
                style: ['subtotals', 'subtotalsBalanceDue', 'noWrap']
            }
        ]
    ];
    data.stack.push(clone);
    ...
    if (hasPayments) {
        ...
        clone.columns[1].table.body = [
            [{
                    text: invoiceLabels.amount_paid,
                    style: ['subtotalsLabel', 'subtotalsBalanceDueLabel']
                },
                {
                    text: formatMoneyInvoice(paymentTotal, invoice),
                    style: ['subtotals', 'subtotalsBalanceDue', 'noWrap']
                }
            ]
        ];
...
NINJA.statementInvoices = function (invoice) {
    ...
    grid[0].push({
        text: invoiceLabels.invoice_number,
        style: ['tableHeader', 'itemTableHeader', 'firstColumn']
    });
    grid[0].push({
        text: invoiceLabels.invoice_date,
        style: ['tableHeader', 'invoiceDateTableHeader']
    });
    grid[0].push({
        text: invoiceLabels.due_date,
        style: ['tableHeader', 'dueDateTableHeader']
    });
    grid[0].push({
        text: invoiceLabels.total,
        style: ['tableHeader', 'totalTableHeader']
    });
    grid[0].push({
        text: invoiceLabels.balance,
        style: ['tableHeader', 'balanceTableHeader', 'lastColumn']
    });
    ...
    for (var i = 0; i < invoice.invoice_items.length; i++) {
        ...
        grid.push([{
                text: item.product_key,
                style: ['invoiceNumber', 'productKey', rowStyle, 'firstColumn']
            },
            {
            ...
            {
                text: formatMoneyInvoice(item.notes, invoice),
                style: ['subtotals', rowStyle]
            },
            {
                text: formatMoneyInvoice(item.cost, invoice),
                style: ['lineTotal', rowStyle, 'lastColumn']
            },
        ]);
    }
...
NINJA.statementPayments = function (invoice) {
    ...
    grid[0].push({
        text: invoiceLabels.payment_date,
        style: ['tableHeader', 'invoiceDateTableHeader']
    });
    grid[0].push({
        text: invoiceLabels.method,
        style: ['tableHeader', 'dueDateTableHeader']
    });
    grid[0].push({
    ...
    for (var i = 0; i < invoice.invoice_items.length; i++) {
        ...
        grid.push([{
            ...
            {
                text: item.custom_value2 ? item.custom_value2 : ' ',
                style: ['dueDate', rowStyle]
            },
            {
                text: notes,
                style: ['notes', rowStyle]
            },
            {
                text: formatMoneyInvoice(item.cost, invoice),
                style: ['lineTotal', rowStyle, 'lastColumn']
...
NINJA.statementAging = function (invoice) {
    ...
    grid[0].push({
        text: '0 - 30',
        style: ['tableHeader', 'alignRight', 'firstColumn']
    });
    grid[0].push({
        text: '30 - 60',
        style: ['tableHeader', 'alignRight']
    });
    grid[0].push({
        text: '60 - 90',
        style: ['tableHeader', 'alignRight']
    ...
    grid[0].push({
        text: '120+',
        style: ['tableHeader', 'alignRight', 'lastColumn']
    });
    ...
    for (var i = 0; i < invoice.invoice_items.length; i++) {
        ...
        grid.push([{
                text: formatMoneyInvoice(item.product_key, invoice),
                style: ['subtotals', 'odd', 'firstColumn']
            },
            {
                text: formatMoneyInvoice(item.notes, invoice),
                style: ['subtotals', 'odd']
            ...
            {
                text: formatMoneyInvoice(item.custom_value1, invoice),
                style: ['subtotals', 'odd']
            },
            {
                text: formatMoneyInvoice(item.custom_value1, invoice),
            ...
            {
                text: formatMoneyInvoice(item.cost, invoice),
                style: ['subtotals', 'odd', 'lastColumn']
            },
        ]);
    }
...
NINJA.invoiceDocuments = function (invoice) {
    ...
    function addDoc(document) {
        ...
        if (path && (window.pdfMake.vfs[path] || document.base64)) {
            ...
            stackItem.columns.push({
                stack: [{
                    image: path,
                    style: 'invoiceDocument',
                    fit: [150, 150]
                }],
                width: 175
            })
...
NINJA.subtotals = function (invoice, hideBalance) {
    ...
    data.push([{
        ...
        text: formatMoneyInvoice(invoice.subtotal_amount, invoice),
        style: ['subtotals', 'subtotal']
    }]);
    ...
    if (invoice.discount_amount != 0) {
        data.push([{
            ...
            text: formatMoneyInvoice(invoice.discount_amount, invoice),
            style: ['subtotals', 'discount']
        }]);
    }
    ...
    if (customValue1 && invoice.custom_taxes1 == '1') {
        data.push([{
            text: customValue1Label,
            style: ['subtotalsLabel', 'customTax1Label']
        }, {
            text: formatMoneyInvoice(invoice.custom_value1, invoice),
            style: ['subtotals', 'customTax1']
        }]);
    }
    if (customValue2 && invoice.custom_taxes2 == '1') {
        data.push([{
            ...
            text: formatMoneyInvoice(invoice.custom_value2, invoice),
            style: ['subtotals', 'customTax2']
        }]);
    }
    ...
    for (var key in invoice.item_taxes) {
        if (invoice.item_taxes.hasOwnProperty(key)) {
            ...
            data.push([{
                ...
                text: formatMoneyInvoice(taxRate.amount, invoice),
                style: ['subtotals', 'tax']
            }]);
        }
    }
    ...
    if (parseFloat(invoice.tax_rate1 || 0) != 0 || invoice.tax_name1) {
        ...
        data.push([{
            ...
            text: formatMoneyInvoice(invoice.tax_amount1, invoice),
    ...
    if (parseFloat(invoice.tax_rate2 || 0) != 0 || invoice.tax_name2) {
        ...
        data.push([{
            ...
            text: formatMoneyInvoice(invoice.tax_amount2, invoice),
            style: ['subtotals', 'tax2']
        }]);
    }
    ...
    if (!invoice.is_quote && invoice.balance_amount >= 0 && (invoice.account.hide_paid_to_date != '1' || paid)) {
        data.push([{
            ...
            text: formatMoneyInvoice(paid, invoice),
            style: ['subtotals', 'paidToDate']
        }]);
    }
    ...
    if (!hideBalance || isPartial) {
        data.push([{
                text: invoice.is_quote || invoice.balance_amount < 0 ? invoiceLabels.total : invoiceLabels.balance_due,
                style: ['subtotalsLabel', isPartial ? '' : 'subtotalsBalanceDueLabel']
            },
            {
                text: formatMoneyInvoice(invoice.total_amount, invoice),
                style: ['subtotals', isPartial ? '' : 'subtotalsBalanceDue']
            }
        ]);
    }
    ...
    if (!hideBalance) {
        if (isPartial) {
            data.push([{
                    text: invoiceLabels.partial_due,
                    style: ['subtotalsLabel', 'subtotalsBalanceDueLabel']
                },
                {
                    text: formatMoneyInvoice(invoice.balance_amount, invoice),
                    style: ['subtotals', 'subtotalsBalanceDue']
                }
            ]);
        }
    }
...
NINJA.subtotalsBalance = function (invoice) {
    ...
    return [
        [{
                text: isPartial ? invoiceLabels.partial_due : (invoice.is_quote || invoice.balance_amount < 0 ? invoiceLabels.total : invoiceLabels.balance_due),
                style: ['subtotalsLabel', 'subtotalsBalanceDueLabel']
            },
            {
                text: formatMoneyInvoice(invoice.balance_amount, invoice),
