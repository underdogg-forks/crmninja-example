    ...
    <div class="panel-body">
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                {!! Former::textarea('private_notes')->rows(! $isRecurring && $account->hasFeature(FEATURE_DOCUMENTS) ?
                6 : 10) !!}
                {!! Former::textarea('public_notes')->rows(! $isRecurring && $account->hasFeature(FEATURE_DOCUMENTS) ?
                6 : 10) !!}
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#expense_date').datepicker('update', '{{ $expense ? Utils::fromSqlDate($expense->expense_date) : '
            new Date()
            ' }}');
        ...
        $('#start_date').datepicker('update',
            '{{ $expense && $expense->start_date ? Utils::fromSqlDate($expense->start_date) : '
            new Date()
            ' }}');
        @elseif(!$expense)
        ...
        $('#mark_paid').click(function (event) {
            if ($('#mark_paid').is(':checked')) {
                ...
                }, "{{ trans('texts.payment_type_' . $account->payment_type->name) }}");
                @endif
            } else {
        ...
        $('.main-form').submit(function () {
            if ($('#document-upload .fallback input').val()) $(this).attr('enctype',
                'multipart/form-data')
    ...
    var ViewModel = function (data) {
        ...
        self.apply_taxes = ko.observable({
            {
                ($expense && ($expense - > tax_name1 || $expense - > tax_name2)) ? 'true' : 'false'
            }
        });
        ...
        @else
        self.convert_currency = ko.observable({
            {
                ($expense && $expense - > isExchanged()) ? 'true' : 'false'
            }
        });
        self.mark_paid = ko.observable({
            {
                $expense && $expense - > isPaid() ? 'true' : 'false'
            }
        });
        @endif
        ...
        self.invoice_documents = ko.observable({
            {
                $expense ? $expense - > invoice_documents : 'invoiceDocuments'
            }
        });
        ...
        self.account_currency_id = ko.observable({
            {
