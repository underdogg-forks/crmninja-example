...
<div class="panel panel-default">
    ...
    <div class="panel-body form-padding-right">
        ...
        {!! Former::populateField('publishable_key', $accountGateway->getPublishableKey() ? str_repeat('*',
        strlen($accountGateway->getPublishableKey())) : '') !!}
        ...
        {!! Former::populateField('plaid_client_id', $accountGateway->getPlaidClientId() ? str_repeat('*',
        strlen($accountGateway->getPlaidClientId())) : '') !!}
        {!! Former::populateField('plaid_secret', $accountGateway->getPlaidSecret() ? str_repeat('*',
        strlen($accountGateway->getPlaidSecret())) : '') !!}
        {!! Former::populateField('plaid_public_key', $accountGateway->getPlaidPublicKey() ? str_repeat('*',
        strlen($accountGateway->getPlaidPublicKey())) : '') !!}
        ...
        <div id="gateway_{{ $gateway->id }}_div" class='gateway-fields' style="display: none">
            ...
            @if ($details && (!$accountGateway || !$accountGateway->getConfigField($field)) && !is_array($details) &&
            !is_bool($details))
            ...
            @endif
            @else
            {!! Former::text($gateway->id.'_'.$field)->label($gateway->id == GATEWAY_STRIPE ? trans('texts.secret_key')
            : ucwords(Utils::toSpaceCase($field))) !!}
            @endif
        ...
        <div class="stripe-ach">
            ...
            ]) : ($accountGateway && $accountGateway->getApplePayEnabled() && Utils::isRootFolder() && !
            $accountGateway->getAppleMerchantId() ? 'verification_file_missing' :
            Utils::isNinjaProd() ? trans('texts.apple_pay_domain', [
            'domain' => $account->subdomain . '.' . APP_DOMAIN, 'link' =>
            link_to('https://dashboard.stripe.com/account/apple_pay', 'Stripe', ['target' => '_blank']),
            ]) : ''))
            ->value(1) !!}
            ...
            ->help(trans('texts.stripe_alipay_help', ['link' =>
            link_to('https://dashboard.stripe.com/account/payments/settings', 'Stripe', ['target' => '_blank'])]))
            ->value(1) !!}
...
<script type="text/javascript">
    function setFieldsShown() {
        ...
        var gateway = _.findWhere(gateways, {
            'id': parseInt(val)
        });
    ...
    $(function () {
        ...
        $('#enable_ach, #enable_alipay, #enable_sofort, #enable_sepa, #enable_bitcoin, #enable_apple_pay').change(
            updateWebhookShown);
