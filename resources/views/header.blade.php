...
<style type="text/css">
    .nav-footer {
        @if (env('TRAVIS')) background-color: #FF0000 !important;
        @elseif (config('mail.driver')=='log'&& ! config('services.postmark')) background-color: #50C878 !important;
        @else background-color: #FD6A02 !important;
        @endif
    }
</style>
@endif
...
<nav class="navbar navbar-default navbar-fixed-top" role="navigation" style="height:60px;">
    ...
    <div class="collapse navbar-collapse" id="navbar-collapse-1">
        <div class="navbar-form navbar-right">
            ...
            {!! Button::success(trans('texts.sign_up'))->withAttributes(array('id' => 'signUpButton', 'onclick' =>
            'showSignUp()', 'style' => 'max-width:100px;;overflow:hidden'))->small() !!} &nbsp;
            @endif
            @elseif (Utils::isNinjaProd() && (!Auth::user()->isPro() || Auth::user()->isTrial()))
            @if (Auth::user()->account->company->hasActivePromo())
            {!! Button::warning(trans('texts.plan_upgrade'))->withAttributes(array('onclick' => 'showUpgradeModal()',
            'style' => 'max-width:100px;overflow:hidden'))->small() !!} &nbsp;
            @else
            {!! Button::success(trans('texts.plan_upgrade'))->withAttributes(array('onclick' => 'showUpgradeModal()',
            'style' => 'max-width:100px;overflow:hidden'))->small() !!} &nbsp;
            @endif
            @endif
            @endif
            ...
            <div class="btn-group user-dropdown">
                ...
                <ul class="dropdown-menu user-accounts">
                    ...
                    @endif
                    <li class="divider"></li>
                    @if (Utils::isAdmin() && Auth::user()->confirmed && Utils::getResllerType() !=
                    RESELLER_ACCOUNT_COUNT)
                    @if (!session(SESSION_USER_ACCOUNTS) || count(session(SESSION_USER_ACCOUNTS)) < 5) <li>{!!
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    {!! trans(Auth::user()->account->getCountTrialDaysLeft() == 0 ? 'texts.trial_footer_last_day' :
                    'texts.trial_footer', [
