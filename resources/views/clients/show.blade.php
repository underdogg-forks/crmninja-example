...
<div class="row">
    ...
    <div class="col-md-5">
        <div class="pull-right">
            ...
            ->withContents([
            ($client->trashed() ? false : ['label' => trans('texts.archive_client'), 'url' =>
            "javascript:onArchiveClick()"]),
            ['label' => trans('texts.delete_client'), 'url' => "javascript:onDeleteClick()"],
            auth()->user()->is_admin ? \DropdownButton::DIVIDER : false,
            auth()->user()->is_admin ? ['label' => trans('texts.purge_client'), 'url' => "javascript:onPurgeClick()"] :
            false,
            ]
            )->split() !!}
            @endcan
    ...
    <div class="panel-body">
        <div class="row">
            ...
            <div class="col-md-3">
                ...
                @if ($client->id_number)
                <p><i class="fa fa-id-number" style="width: 20px"></i>{{ trans('texts.id_number').':
                    '.$client->id_number }}</p>
                @endif
                @if ($client->vat_number)
                <p><i class="fa fa-vat-number" style="width: 20px"></i>{{ trans('texts.vat_number').':
                    '.$client->vat_number }}</p>
                @endif

                @if ($client->account->customLabel('client1') && $client->custom_value1)
                {{ $client->account->present()->customLabel('client1') . ': ' }} {!! nl2br(e($client->custom_value1))
                !!}<br />
                @endif
                @if ($client->account->customLabel('client2') && $client->custom_value2)
                {{ $client->account->present()->customLabel('client2') . ': ' }} {!! nl2br(e($client->custom_value2))
                !!}<br />
                @endif
            ...
            <div class="col-md-3">
                ...
                <i class="fa fa-dashboard" style="width: 20px"></i><a href="{{ $contact->link }}" onclick="window.open('{{ $contact->link }}?silent=true', '_blank');return false;">{{
                    trans('texts.view_in_portal') }}</a>
                ...
                <h3>{{ trans('texts.standing') }}
                    <table class="table" style="width:100%">
                        <tr>
                            ...
                            <td style="text-align: right">{{ Utils::formatMoney($client->paid_to_date,
                                $client->getCurrencyId()) }}</td>
                        </tr>
                        <tr>
                            <td><small>{{ trans('texts.balance') }}</small></td>
                            <td style="text-align: right">{{ Utils::formatMoney($client->balance,
                                $client->getCurrencyId()) }}</td>
                        </tr>
                        @if ($credit > 0)
                        <tr>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer" id="signUpFooter" style="margin-top: 0px">
                ...
                <button type="button" class="btn btn-danger" onclick="onReactivateClick()" id="reactivateButton" style="display:none;">{{
                    trans('texts.reactivate') }} </button>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $(function () {
        $('.normalDropDown:not(.dropdown-toggle)').click(function (event) {
            openUrlOnClick('{{ URL::to('
                clients / ' . $client->public_id . ' / edit ') }}', event);
        });
        $('.primaryDropDown:not(.dropdown-toggle)').click(function (event) {
            openUrlOnClick('{{ URL::to('
                clients / statement / ' . $client->public_id ) }}', event);
        });
    ...
    function onPurgeClick() {
        sweetConfirm(function () {
            ...
            n\\ n " . trans('texts.mobile_refresh_warning') . "\\
            n\\ n " . trans('texts.no_undo') }}");
    }
    ...
    function initialize() {
        ...
        var address = {!!json_encode(e(
                "{$client->address1} {$client->address2} {$client->city} {$client->state} {$client->postal_code} "
                .($client - > country ? $client - > country - > getName() : ''))) !!
        };
