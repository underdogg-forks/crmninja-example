...
<script>
    ...
    function getPDFString(cb) {
        ...
        invoice.features = {
            customize_invoice_design: {
                {
                    $account - > hasFeature(FEATURE_CUSTOMIZE_INVOICE_DESIGN) ? 'true' : 'false'
                }
            },
            remove_created_by: {
                {
                    $account - > hasFeature(FEATURE_REMOVE_CREATED_BY) ? 'true' : 'false'
                }
            },
            invoice_settings: {
                {
                    $account - > hasFeature(FEATURE_INVOICE_SETTINGS) ? 'true' : 'false'
                }
            }
        };
    ...
    function onDownloadClick() {
        ...
        doc.save("{{ str_replace(' ', '_', trim($client->getDisplayName())) . '-' . trans('texts.statement') }}" +
            '.pdf');
    }
</script>
