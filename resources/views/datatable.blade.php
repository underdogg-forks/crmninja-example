<table class="table table-striped data-table {{ $class = str_random(8) }}">
    <colgroup>
        @for ($i = 0; $i
        < count($columns); $i++) <col class="con{{ $i }}" />
        @endfor
    </colgroup>
    <thead>
        <tr>
            @foreach($columns as $i => $c)
            <th align="center" valign="middle" class="head{{ $i }}" @if ($c=='checkbox' ) style="width:20px" @endif>
...
<script type="text/javascript">
    ...
    function refreshDatatable {
        {
            isset($values['entityType']) ? '_'.$values['entityType'] : ''
        }
    }() {
        window['dataTable{{ isset($values['
            entityType ']) ? '
            _ ' . $values['
            entityType '] : '
            ' }}'].api().ajax.reload();
        ...
        window['dataTable{{ isset($values['
            ...
            "aoColumnDefs": [
                ...
                @endif {
                    ...
                    'aTargets': {
                        {
                            isset($values['rightAlign']) ? json_encode($values['rightAlign']) : '[]'
                        }
                    }
                }
            ],
            ...
            @endforeach "fnDrawCallback": function (oSettings) {
                @if(isset($values['entityType']))
                if (window.onDatatableReady_ {
                        {
                            $values['entityType']
                        }
                    }) {
