...
<ol class="breadcrumb">
	...
	<li>{!! link_to(($entityType == ENTITY_QUOTE ? 'recurring_quotes' : 'recurring_invoices'), trans('texts.' .
		($entityType == ENTITY_QUOTE ? 'recurring_quotes' : 'recurring_invoices'))) !!}</li>
	@else
	<li>{!! link_to(($entityType == ENTITY_QUOTE ? 'quotes' : 'invoices'), trans('texts.' . ($entityType == ENTITY_QUOTE ?
		'quotes' : 'invoices'))) !!}</li>
...
<div data-bind="with: invoice">
			...
			<div class="row" style="min-height:195px" onkeypress="formEnterClick(event)">
						...
						<div class="form-group" style="margin-bottom: 8px">
							<div class="col-lg-8 col-sm-8 col-lg-offset-4 col-sm-offset-4">
								...
								<span data-bind="visible: $root.invoice().client().public_id() > 0" style="display:none">|
									<a data-bind="attr: {href: '{{ url('/clients') }}/' + $root.invoice().client().public_id()}" target="_blank">{{
										trans('texts.view_client') }}</a>
								</span>
							</div>
						</div>
						...
						<div style="display:none" class="form-group" data-bind="visible: contacts().length > 0, foreach: contacts">
							<div class="col-lg-8 col-lg-offset-4 col-sm-offset-4">
								...
								<span data-bind="visible: !$root.invoice().is_recurring()">
									...
									<span style="vertical-align:text-top;color:red" class="fa fa-exclamation-triangle" data-bind="visible: $data.email_error, tooltip: {title: $data.email_error}"></span>
									<span style="vertical-align:text-top;padding-top:2px" class="fa fa-info-circle" data-bind="visible: $data.invitation_status, tooltip: {title: $data.invitation_status, html: true},
	                                    style: {color: $data.info_color}"></span>
									<span class="signature-wrapper">&nbsp;
										<span style="vertical-align:text-top;color:#888" class="fa fa-user" data-bind="visible: $data.invitation_signature_svg, tooltip: {title: $data.invitation_signature_svg, html: true}"></span>
									</span>
									@endif
								</span>
								@endif
				...
				<div class="col-md-4" id="col_2">
					<div data-bind="visible: !is_recurring()">
						{!! Former::text('invoice_date')->data_bind("datePicker: invoice_date, valueUpdate:
						'afterkeydown'")->label($account->getLabel("{$entityType}_date"))
						->data_date_format(Session::get(SESSION_DATE_PICKER_FORMAT,
						DEFAULT_DATE_PICKER_FORMAT))->appendIcon('calendar')->addGroupClass('invoice_date') !!}
						{!! Former::text('due_date')->data_bind("datePicker: due_date, valueUpdate:
						'afterkeydown'")->label($account->getLabel($invoice->getDueDateLabel()))
						->placeholder($invoice->id || $invoice->isQuote() ? ' ' : $account->present()->dueDatePlaceholder())
						->data_date_format(Session::get(SESSION_DATE_PICKER_FORMAT,
						DEFAULT_DATE_PICKER_FORMAT))->appendIcon('calendar')->addGroupClass('due_date') !!}
					...
					<div data-bind="visible: is_recurring" style="display: none">
						...
						{!! Former::text('start_date')->data_bind("datePicker: start_date, valueUpdate:
						'afterkeydown'")->data_date_start_date($invoice->id ? false : $account->formatDate($account->getDateTime()))
						->data_date_format(Session::get(SESSION_DATE_PICKER_FORMAT,
						DEFAULT_DATE_PICKER_FORMAT))->appendIcon('calendar')->addGroupClass('start_date') !!}
						{!! Former::text('end_date')->data_bind("datePicker: end_date, valueUpdate:
						'afterkeydown'")->data_date_start_date($invoice->id ? false : $account->formatDate($account->getDateTime()))
						->data_date_format(Session::get(SESSION_DATE_PICKER_FORMAT,
						DEFAULT_DATE_PICKER_FORMAT))->appendIcon('calendar')->addGroupClass('end_date') !!}
						{!!
						Former::select('recurring_due_date')->label($account->getLabel($invoice->getDueDateLabel()))->options($recurringDueDates)->data_bind("value:
						recurring_due_date")->appendIcon('question-sign')->addGroupClass('recurring_due_date') !!}
					</div>
					@endif
				...
				<div class="col-md-4" id="col_2">
					...
					<span data-bind="visible: is_recurring()" style="display: none">
						<div data-bind="visible: !(auto_bill() == {{AUTO_BILL_OPT_IN}} &amp;&amp; client_enable_auto_bill()) &amp;&amp; !(auto_bill() == {{AUTO_BILL_OPT_OUT}} &amp;&amp; !client_enable_auto_bill())"
						 style="display: none">
							...
							->data_bind("value: auto_bill, valueUpdate: 'afterkeydown',
							event:{change:function(){if(auto_bill()==".AUTO_BILL_OPT_IN.")client_enable_auto_bill(0);if(auto_bill()==".AUTO_BILL_OPT_OUT.")client_enable_auto_bill(1)}}")
							->options([
					...
					@endif
					{!! Former::text('po_number')->label($account->getLabel('po_number', 'po_number_short'))->data_bind("value:
					po_number, valueUpdate: 'afterkeydown'") !!}
					...
					<div class="form-group" style="margin-bottom: 8px">
						<div class="col-lg-8 col-sm-8 col-sm-offset-4 smaller" style="padding-top: 10px;">
							...
							{!! trans('texts.created_by_invoice', ['invoice' => link_to('/invoices/'.$invoice->recurring_invoice->public_id,
							trans('texts.recurring_invoice'))]) !!}
							<p />
							@elseif ($invoice->id)
							@if (isset($lastSent) && $lastSent)
							{!! trans('texts.last_sent_on', ['date' => link_to('/invoices/'.$lastSent->public_id, $invoice->last_sent_date,
							['id' => 'lastSent'])]) !!}
							<p />
							@endif
							@if ($invoice->is_recurring && $invoice->start_date && $invoice->is_public)
							@if ($sendNextDate = $invoice->getNextSendDate())
							{!! trans('texts.next_send_on', ['date' => '<span data-bind="tooltip: {title: \''.$invoice->getPrettySchedule().'\', html: true}">'
								. $account->formatDate($sendNextDate).
			...
			<div class="table-responsive" style="padding-top:4px;">
				...
				<table class="pull-right subtotals-table" style="margin-right:40px; margin-top:0px;">
					...
					<tr data-bind="style: { 'font-weight': partial() ? 'normal' : 'bold', 'font-size': partial() ? '1em' : '1.05em' }"
					 style="font-size:1.05em;font-weight:bold;">
						<td class="hide-border" data-bind="css: {'hide-border': !partial()}" colspan="2">{{ $entityType == ENTITY_INVOICE
							? $invoiceLabels['balance_due'] : trans('texts.total') }}</td>
				...
				<div role="tabpanel" class="pull-left" style="margin-left:40px; margin-top:30px;">
					...
					<ul class="nav nav-tabs" role="tablist" style="border: none">
						<li role="presentation" class="active"><a href="#public_notes" aria-controls="notes" role="tab" data-toggle="tab">{{
								trans('texts.public_notes') }}</a></li>
						<li role="presentation"><a href="#private_notes" aria-controls="terms" role="tab" data-toggle="tab">{{
								trans("texts.private_notes") }}</a></li>
						<li role="presentation"><a href="#terms" aria-controls="terms" role="tab" data-toggle="tab">{{
								trans("texts.terms") }}</a></li>
						...
						<div role="tabpanel" class="tab-pane" id="terms">
							...
							->help('<div class="checkbox">
								...
								<div class="pull-right" data-bind="visible: showResetTerms()">
									<a href="#" onclick="return resetTerms()" title="'. trans('texts.reset_terms_help') .'">' .
										trans("texts.reset_terms") . '</a>
								</div>
							</div>') !!}
						</div>
						<div role="tabpanel" class="tab-pane" id="footer">
							...
							->help('<div class="checkbox">
								...
								<div class="pull-right" data-bind="visible: showResetFooter()">
									<a href="#" onclick="return resetFooter()" title="'. trans('texts.reset_footer_help') .'">' .
										trans("texts.reset_footer") . '</a>
								</div>
							</div>') !!}
						</div>
						@if ($account->hasFeature(FEATURE_DOCUMENTS))
	...
	<center class="buttons">
		...
		{!! Former::select('invoice_design_id')->style('display:inline;width:150px;background-color:white
		!important')->raw()->fromQuery($invoiceDesigns, 'name', 'id')->data_bind("value:
		invoice_design_id")->addOption(trans('texts.more_designs') . '...', '-1') !!}
		@else
		{!! Former::select('invoice_design_id')->style('display:inline;width:150px;background-color:white
		!important')->raw()->fromQuery($invoiceDesigns, 'name', 'id')->data_bind("value: invoice_design_id") !!}
		@endif
		...
		@if (! $invoice->trashed())
		{!! Button::info(trans("texts.email_{$entityType}"))->withAttributes(array('id' => 'emailButton', 'onclick' =>
		'onEmailClick()'))->appendIcon(Icon::create('send')) !!}
		...
		@if ($invoice->isSent())
		{!! Button::success(trans("texts.save_{$entityType}"))->withAttributes(array('id' => 'saveButton', 'onclick' =>
		'onSaveClick()'))->appendIcon(Icon::create('floppy-disk')) !!}
		@else
		{!! Button::normal(trans("texts.save_draft"))->withAttributes(array('id' => 'draftButton', 'onclick' =>
		'onSaveDraftClick()'))->appendIcon(Icon::create('floppy-disk')) !!}
		@if (! $invoice->trashed())
		{!! Button::success(trans($invoice->is_recurring ? "texts.mark_ready" :
		"texts.mark_sent"))->withAttributes(array('id' => 'saveButton', 'onclick' =>
		'onMarkSentClick()'))->appendIcon(Icon::create('globe')) !!}
		@endif
		@endif
		@if (! $invoice->trashed())
		{!! Button::info(trans("texts.email_{$entityType}"))->withAttributes(array('id' => 'emailButton', 'onclick' =>
		...
		{!! Button::normal(trans($invoice->is_recurring ? 'texts.disable_recurring' :
		'texts.enable_recurring'))->withAttributes(['id' => 'recurrButton', 'onclick' =>
		'onRecurrClick()'])->appendIcon(Icon::create('repeat')) !!}
		@elseif (! empty($tasks))
		{!! Button::normal(trans('texts.add_product'))->withAttributes(['id' => 'addItemButton', 'onclick' =>
		'onAddItemClick()'])->appendIcon(Icon::create('plus-sign')) !!}
		@endif
		@endif
		@if ($invoice->trashed())
		{!! Button::primary(trans('texts.restore'))->withAttributes(['onclick' =>
		'submitBulkAction("restore")'])->appendIcon(Icon::create('cloud-download')) !!}
		@endif
		@endif
		@endif
	...
	<div style="font-size:larger">
		{!! trans('texts.pro_plan_remove_logo', ['link'=>'<a href="javascript:showUpgradeModal()">' .
		...
		<div class="modal-dialog" data-bind="css: {'large-dialog': $root.showMore}">
			<div class="modal-content" style="background-color: #f8f8f8">
							...
							<div class="row" data-bind="with: client" onkeypress="clientModalEnterClick(event)">
								...
								<div style="margin-left:0px;margin-right:0px" data-bind="css: {'col-md-6': $root.showMore}">
									...
									<div data-bind='template: { foreach: contacts,
										...
										{!! Former::password('password')->data_bind("value: (typeof
										password=='function'?password():null)?'-%unchanged%-':'', valueUpdate: 'afterkeydown',
										attr: {name: 'client[contacts][' + \$index() +
										'][password]'}")->autocomplete('new-password')->data_lpignore('true') !!}
										@endif
										@if (Auth::user()->hasFeature(FEATURE_INVOICE_SETTINGS))
				...
				<div class="modal-footer">
					<span class="error-block" id="emailError" style="display:none;float:left;font-weight:bold">{{
						trans('texts.provide_name_or_email') }}</span><span>&nbsp;</span>
					...
					<button id="clientDoneButton" type="button" class="btn btn-primary" data-bind="click: $root.clientFormComplete">{{
						trans('texts.done') }}</button>
				</div>
	...
	<div class="modal fade" id="recurringDueDateModal" tabindex="-1" role="dialog" aria-labelledby="recurringDueDateModalLabel"
	 aria-hidden="true">
		<div class="modal-dialog" style="min-width:150px">
			<div class="modal-content">
				<div class="modal-header">
...
<script type="text/javascript">
	...
	$(function () {
		...
		model.invoice().is_recurring({
			{
				$invoice - > is_recurring ? '1' : '0'
			}
		});
		...
		NINJA.formIsChanged = true;
		model.expense_currency_id({
			{
				isset($expenseCurrencyId) ? $expenseCurrencyId : 0
			}
		});
		...
		$(
			'#invoice_footer, #terms, #public_notes, #invoice_number, #invoice_date, #due_date, #partial_due_date, #start_date, #po_number, #discount, #currency_id, #invoice_design_id, #recurring, #is_amount_discount, #partial, #custom_text_value1, #custom_text_value2'
		).change(function () {
		...
		if (model.invoice().client().public_id() || {
				{
					$invoice - > id || count($clients) == 0 ? '1' : '0'
				}
			}) {
			// do nothing
		} else {
	...
	function createInvoiceModel() {
		...
		invoice.is_quote = {
			{
				$entityType == ENTITY_QUOTE ? 'true' : 'false'
			}
		};
		invoice.contact = _.findWhere(invoice.client.contacts, {
			send_invoice: true
		});
		...
		invoice.imageWidth = {
			{
				$account - > getLogoWidth()
			}
		};
		invoice.imageHeight = {
			{
				$account - > getLogoHeight()
			}
		};
		@endif
	...
	function onDownloadClick() {
		...
		var type = invoice.is_quote ? {!!json_encode(trans('texts.'.ENTITY_QUOTE)) !!
		} : {!!json_encode(trans('texts.'.ENTITY_INVOICE)) !!
		};
	...
	function onSaveClick() {
		...
		if (model.invoice().is_recurring()) {
			if (model.invoice().start_date() != model.invoice().start_date_orig()) {
				var text = {!!json_encode(trans("texts.original_start_date")) !!
					} + ': ' + model.invoice().start_date_orig() + '\n' +
					{!!json_encode(trans("texts.new_start_date")) !!
					} + ': ' + model.invoice().start_date();
				@if($account - > auto_email_invoice)
	...
	function onFormSubmit(event) {
		...
		var clientCurrencyId = model.invoice().client().currency_id() || {
			{
	...
	function handleSaveFailed(data) {
		...
		swal({!!json_encode(trans('texts.invoice_save_error')) !!
		}, error);
	}
	...
	function onPaymentClick() {
		...
		sweetConfirm(function () {
			...
			payments / create / ' . $invoice->client->public_id . ' / ' . $invoice->public_id ) }}';
	...
	function onCreditClick() {
		window.location = '{{ URL::to('
		credits / create / ' . $invoice->client->public_id . ' / ' . $invoice->public_id ) }}';
	}
	@endif
	...
	function onPartialChange() {
		...
		if (val != oldVal) {
			...
			$('.partial')
				.addClass('has-error')
				.find('div.partial')
				.append('<span class="help-block">{{ trans('
					texts.partial_value ') }}</span>');
		} else {
			$('.partial')
	...
	function setInvoiceNumber(client) {
		...
		number = number.replace('{$clientCounter}', pad(client.quote_number_counter, {
			{
				$account - > invoice_number_padding
			}
		}));
		@else
		number = number.replace('{$clientCounter}', pad(client.invoice_number_counter, {
			{
				$account - > invoice_number_padding
			}
		}));
